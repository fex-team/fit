...
function getPathModules (type) {
    return moduleGlobal[type + 'Modules'] = fs.readdirSync(path.join(root, 'lib', type)).filter((name) => {
...
switch (args[0]) {
    ...
    case 'initsubmodule':
        ...
        moduleDistribute((modules) => {
            modules.map((abPath) => {
                let subPath = abPath.replace(root, '.')
...
function initPrepare () {
    ...
    let allConfigModules = getAllConfigModules()
    let allPathModules = getAllPathModules()
...
function addSubModule (modules) {
    ...
    modules.forEach((val) => {
        ...
        let repoPath = `ssh://g@gitlab.baidu.com:8022/tb-component/${gitlabPrefix}-${moduleName}.git`
