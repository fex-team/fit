...
export default class Table extends React.Component {
    ...
    handleSortChange(key, isSort) {
        ...
        this.setState({
            ...
            let searchParam = this.props.onSort(key)
    ...
    render() {
        let Th = this.state.fields.map((item, index)=> {
            ...
            default:
                ...
                return (
                    <th key={index} className={thClass} onClick={this.handleSortChange.bind(this,item.key,item.sort)}>
                        {item.value}
                ...
                default:
                    if (field.edit) {
                        if (index === this.state.showInputPosition.x && fieldIndex === this.state.showInputPosition.y) {
                            return (
                                <td key={fieldIndex} style={{padding:0,border:'none'}} id="edit-input">
                                    <div style={{display:'flex'}}>
                                        <Input value={this.state.editValue||tr[field.key]}
                                               onChange={this.handleChangeEditValue.bind(this)}/>
                                        <Button type="success" style={{borderRadius:0,marginLeft:-1}}
                        ...
                        return (
                            <td key={fieldIndex} onDoubleClick={this.handleTdDoubleClick.bind(this,index,fieldIndex)}>
                                {tr[field.key]}
                            </td>
                        )
                    }
                ...
                <div className="panel">
                    <div className="panel-heading">
                        ...
                        {this.props.add.url === '' ? null :
                            <Add fields={this.props.fields} opts={this.props.add} onAdd={this.handleAdd.bind(this)}/>
                        }
                        <span className={infoClass} style={{marginLeft:10}}>{this.state.info.message}</span>
                    </div>
